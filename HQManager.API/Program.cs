...
builder.Services.AddScoped<IPersonagemRepository>(sp =>
{
    var dbContext = sp.GetRequiredService<MongoDbContext>();
    return new PersonagemRepository(dbContext.Personagens);
});

builder.Services.AddScoped<IEquipeRepository>(sp =>
{
    var dbContext = sp.GetRequiredService<MongoDbContext>();
...
builder.Services.AddScoped<IEdicaoRepository>(sp =>
{
    var dbContext = sp.GetRequiredService<MongoDbContext>();
    return new EdicaoRepository(dbContext.Edicoes);
});

builder.Services.AddScoped<IDashboardRepository>(sp =>
{
    var dbContext = sp.GetRequiredService<MongoDbContext>();
    return new DashboardRepository(dbContext.Database);
});
...
builder.Services.AddCors(options =>
{
    options.AddPolicy("AllowFrontend",
        policy =>
        {
            policy.WithOrigins("http://localhost:4200") // endereço do frontend
                  .AllowAnyHeader()
                  .AllowAnyMethod();
        });
});
